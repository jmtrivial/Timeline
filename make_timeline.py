...
class Timeline:
	def __init__(self, filename):
		...
		self.datetime0 = Timeline.datetime_from_dateordatetime(self.start_date[0]) - padding
		self.datetime1 = Timeline.datetime_from_dateordatetime(self.end_date[0]) + padding
		self.total_secs = (self.datetime1 - self.datetime0).total_seconds()	

	def datetime_from_string(self, s):
		dt, flag = self.cal.parse(s.split("+")[0])
		if flag == 3:
			dt = datetime.datetime(*dt[:6])
		elif flag == 1:
			dt = datetime.date(*dt[:3])
	...
	@staticmethod
	def datetime_from_dateordatetime(s):
	...
	def getSortedDates(self):
		...
		for callout in callouts_data:
			...
			event_date_time = Timeline.datetime_from_dateordatetime(event_date[0])
			event_color = callout[2] if len(callout) > 2 else Colors.black
			event_bold = "bold" if len(callout) > 3 and callout[3] else "normal"
			sorted_dates.append(event_date_time)
			if event_date_time not in inv_callouts:
				inv_callouts[event_date_time] = []
			prefix = "~ " if callout[1][0] == '~' else ""
			inv_callouts[event_date_time].append((event, event_color, prefix, event_bold, event_date))
		sorted_dates.sort()
		return sorted_dates, inv_callouts

class TimelineICAL(Timeline):
	def __init__(self, filename):
		Timeline.__init__(self, filename)
		import icalendar

		self.ical = icalendar.Calendar()
		self.ical.add('prodid', '-//Generated by Timeline//mxm.dk//')
	...
	def build(self):
		import icalendar
		import pytz
		self.ical.add('dtstart', self.start_date[0])
		self.ical.add('dtend', self.end_date[0])

		sdates, inv_callouts = self.getSortedDates()
		i = 0
		for d in sdates:
			e = icalendar.Event()
			event, event_color, prefix, event_bold, event_date_time = inv_callouts[d].pop()
			e.add("summary", event)
			e.add("dtstart", event_date_time[0])
			e.add("uid", icalendar.vDatetime(d).to_ical() + "-" + str(i) + "@activmap.limos.fr")
			if isinstance(event_date_time, datetime.datetime):
				e.add("dtend", event_date_time[0])
...
class TimelineSVG (Timeline):
	...
	def to_string(self):
		return self.drawing.tostring().encode('utf-8')
	...
	def create_eras(self, y_era, y_axis, height):
		...
		for era in eras_data:
			...
			percent_width0 = (t0[0] - self.datetime0).total_seconds()/self.total_secs
			percent_width1 = (t1[0] - self.datetime0).total_seconds()/self.total_secs
	...
	def add_axis_label(self, dt, label, **kwargs):
		...
		d = Timeline.datetime_from_dateordatetime(dt[0])
		percent_width = (d - self.datetime0).total_seconds()/self.total_secs
	...
	def create_callouts(self):
		...
		if 'callouts' not in self.data:
			return
		sorted_dates, inv_callouts = self.getSortedDates()
				
		# add callouts, one by one, making sure they don't overlap
		prev_x = [float('-inf')]
		prev_level = [-1]
		for event_date_time in sorted_dates:
			event, event_color, prefix, event_bold, event_date = inv_callouts[event_date_time].pop()
			num_sec = (event_date_time - self.datetime0).total_seconds()
...
if __name__ == '__main__':
	...
	if outfilename.endswith(".svg"):
		timeline = TimelineSVG(filename)
	elif outfilename.endswith(".ics") or outfilename.endswith(".ical"):
		timeline = TimelineICAL(filename)
	else:
		print 'Unknown output format: ', outfilename
		print 'Please choose one of the following supported formats: svg, ics'
		sys.exit(-2) 

	timeline.build()

	with open(outfilename, 'w') as outfile:
		outfile.write(timeline.to_string())
